...
/* Canada cable, Rogers */
static struct CHANLIST ntsc_cable_ca_rogers[] = {
    { "2",       55250 },
    { "3",       61250 },
    { "4",       67250 },
    { "5",       77250 },
...
struct CHANLISTS chanlists[] = {
    ...
    { "canada-cable-rogers",     ntsc_cable_ca_rogers,     CHAN_COUNT(ntsc_cable_ca_rogers)     },
...
for(i=0;(chanlists[i].name!=NULL);i++)
	Tcl_ListObjAppendElement(interp, ans, Tcl_NewStringObj(chanlists[i].name, strlen(chanlists[i].name)));
...
if(argc<2){
	Tcl_AppendResult(interp, "ERROR: freq_get_channel_list requires one argument", NULL);
	return TCL_ERROR;
	}

table=atoi(argv[1]);
if((table<0)||(table>(sizeof(chanlists)/sizeof(struct CHANLISTS)))||(chanlists[table].name==NULL)){
	Tcl_AppendResult(interp, "ERROR: freq_get_channel_list: no such channel table", NULL);
	return TCL_ERROR;
	}
ans=Tcl_NewListObj(0, NULL);
for(i=0;i<chanlists[table].count;i++)
	Tcl_ListObjAppendElement(interp, ans, Tcl_NewStringObj(chanlists[table].list[i].name, strlen(chanlists[table].list[i].name)));
Tcl_SetObjResult(interp, ans);
...
if(argc<3){
	Tcl_AppendResult(interp, "ERROR: freq_get_channel_value requires two arguments", NULL);
	return TCL_ERROR;
	}

table=atoi(argv[1]);
if((table<0)||(table>=(sizeof(chanlists)/sizeof(struct CHANLISTS)))||(chanlists[table].name==NULL)){
	Tcl_AppendResult(interp, "ERROR: freq_get_channel_value: no such channel table", NULL);
	return TCL_ERROR;
	}
channel=atoi(argv[2]);
...
Tcl_SetObjResult(interp, Tcl_NewIntObj((chanlists[table].list[channel].freq*16)/1000));
...
struct {
	...
	{"freq_get_table_list", freq_get_table_list},
	{"freq_get_channel_list", freq_get_channel_list},
	{"freq_get_channel_value", freq_get_channel_value},
	{NULL, NULL}
	};
...
{
long i;
for(i=0;freq_commands[i].name!=NULL;i++)
	Tcl_CreateCommand(interp, freq_commands[i].name, freq_commands[i].command, (ClientData)0, NULL);
}
