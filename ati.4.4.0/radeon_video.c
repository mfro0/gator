...
void RADEON_TDA9885_Init(RADEONPortPrivPtr);
void RADEON_TDA9885_SetEncoding(RADEONPortPrivPtr);
...
static void RADEONInitI2C(ScrnInfoPtr pScrn, RADEONPortPrivPtr pPriv)
{
    ...
    xf86DrvMsg(pScrn->scrnIndex, X_INFO, "ref=%d M=0x%02x N=0x%02x timing=0x%02x\n", (int)pll->reference_freq, (int)pPriv->radeon_M, (int)pPriv->radeon_N, (int)pPriv->radeon_i2c_timing);
#if 0
    pPriv->radeon_M=0x32;
    pPriv->radeon_N=0x33;
    pPriv->radeon_i2c_timing=2*pPriv->radeon_N;
...
RADEONSetupImageVideo(ScreenPtr pScreen)
{
    ...
    pPriv = (RADEONPortPrivPtr)(adapt->pPortPrivates[0].ptr);
    /* XXX REGION_INIT(pScreen, &(pPriv->clip), NullBox, 0); */
...
/*   RADEONWaitForFifo(pScrn, 15); */
   ...
   if(! pPriv->video_stream_active)
   {
      ...
      RADEONWaitForIdleMMIO(pScrn);
      xf86DrvMsg(pScrn->scrnIndex, X_INFO, "RADEON_VIDEOMUX_CNT=0x%08lx\n", INREG(RADEON_VIDEOMUX_CNTL));
   ...
   xf86DrvMsg(pScrn->scrnIndex, X_INFO, "OV0_FLAG_CNTL=0x%08lx\n", INREG(RADEON_OV0_FLAG_CNTL));
/*   OUTREG(RADEON_OV0_FLAG_CNTL, 8); */
   xf86DrvMsg(pScrn->scrnIndex, X_INFO, "OV0_VID_BUFFER_CNTL=0x%08lx\n", INREG(RADEON_VID_BUFFER_CONTROL));
   xf86DrvMsg(pScrn->scrnIndex, X_INFO, "CAP0_BUF_STATUS=0x%08lx\n", INREG(RADEON_CAP0_BUF_STATUS));
